...
2. **Automation stack** – code lives in `src/browserMode.ts` and is a lightly refactored version of the `oraclecheap` utility:
   ...
   - Navigates to `chatgpt.com`, switches the model to the requested ChatGPT picker row (`Latest`, `Thinking`, `Instant`, etc.), pastes the prompt, waits for completion, and copies the markdown via the built-in “copy turn” button.
...
- `--browser-thinking-time <light|standard|extended|heavy>`: set the ChatGPT thinking-time intensity (Thinking/Pro models only). Pro browser runs default to the extended Pro composer mode when Oracle selects the model; use this flag to override it, or `--browser-model-strategy current` to leave the current composer mode alone. You can also set a default in `~/.oracle/config.json` via `browser.thinkingTime`.
...
- **Model picker debugging:** If the ChatGPT model dropdown flickers or selection fails, set `ORACLE_BROWSER_MODEL_DEBUG=1` (or use `-v/--verbose`) before `oracle --engine browser`. Oracle logs a `dom-probe` line with `[data-testid="model-switcher-dropdown-button"]` state (`aria-expanded`, `aria-controls`, …), counts for Radix/menu surfaces, related `data-testid`s, and any open menu preview. On failure it also prints a compact `dom-compact` line without those flags. If you see `model trigger re-clicked N time(s)`, the site isn’t matching our “menu open” detection and keeps toggling the trigger.
- **Model menu settle:** After opening the model picker, Oracle waits ~1s before reading options — the first frame is often incomplete; then it picks Pro (etc.). If the menu auto-closed during that wait, it reopens once before selecting.
- **Menu still open:** After selection, Oracle runs `closeOpenModelMenuBestEffort`: click the Pro menu row again (matches “Latest / Instant / Thinking / Pro” text), then trusted mouse clicks just outside the menu and Escape via CDP so the composer isn’t covered.
- **Logs / root cause:** Every browser run logs `[browser] [model] diagnostic` **before** and **after** closing the menu. Check `pickedHasSwitcher: true` — if it was `false` while stuck, Oracle was targeting the wrong `[role="menu"]` (e.g. sidebar). The picker is chosen by visible area + `model-switcher-*` nodes under the header trigger. Session log: `oracle session <id>` or `~/.oracle/sessions/<id>/log`.
...
- `--model`: the same flag used for API runs is accepted, but browser Pro aliases (`latest`, `pro`, `gpt-5.5-pro`, older `gpt-5.x-pro` aliases) resolve to ChatGPT’s `Latest` picker target. Use `gpt-5.5-pro`/`latest` for current Pro, `gpt-5.5` or `gpt-5.4` for Thinking, and `gpt-5.2-thinking` / `gpt-5.2-instant` for legacy picker rows.
...
- **Model picker drift** – we rely on heuristics to pick ChatGPT picker rows such as `Latest`, `Thinking`, and `Instant`. If OpenAI changes the DOM we need to refresh the selectors quickly. Consider snapshot tests or a small “self check” command.
