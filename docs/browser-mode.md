...
- `--browser-thinking-time <light|standard|extended|heavy>`: set thinking-time intensity for non-GPT browser models. GPT browser runs always select ChatGPT Latest and force Extended Pro composer mode; stale caller labels such as `GPT-5.4 Pro` or `standard` thinking are ignored.
