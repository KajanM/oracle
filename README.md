...
Oracle bundles your prompt and files so another AI can answer with real context. Browser mode targets ChatGPT’s latest Pro model by selecting the picker’s “Latest” row (currently Pro 5.5 with Extended thinking). API mode still supports explicit model IDs such as `gpt-5.4-pro`, `gpt-5.4`, `gpt-5.2`, `gpt-5.1-codex`, Gemini 3.1 Pro, Gemini 3 Pro, Claude Sonnet 4.5, Claude Opus 4.1, and more—and it can ask one or multiple API models in a single run. Use `--browser-model-strategy current` to keep the active ChatGPT model, or `ignore` to skip the picker.

## Setting up (macOS Browser Mode)

Browser mode lets you use ChatGPT’s latest Pro model without any API keys — it automates your Chrome browser directly.
...
| `-m, --model <name>`                                            | Browser Pro aliases select ChatGPT “Latest” (currently Pro 5.5). API built-ins include `gpt-5.4-pro` default, `gpt-5.4`, `gpt-5.1-pro`, `gpt-5-pro`, `gpt-5.1`, `gpt-5.1-codex`, `gpt-5.2`, `gpt-5.2-instant`, `gpt-5.2-pro`, `gemini-3.1-pro` API-only, `gemini-3-pro`, `claude-4.5-sonnet`, `claude-4.1-opus`, plus any OpenRouter id. |
...
| `--browser-thinking-time <light\|standard\|extended\|heavy>`    | Set non-GPT browser thinking-time intensity. GPT browser runs always target ChatGPT Latest / Extended Pro.                                                                                                                                                                                                                               |
...
```json5
{
  model: "gpt-5.5-pro", // browser alias for ChatGPT Latest / Pro 5.5
  engine: "api",
  filesReport: true,
  browser: {
