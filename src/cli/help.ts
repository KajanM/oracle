...
function renderHelpBanner(version: string, colors: HelpColors): string {
  const subtitle =
    "Prompt + files required — latest Pro browser mode or API models for tough code/file context.";
...
function renderHelpFooter(program: Command, colors: HelpColors): string {
  const tips = [
    ...
    `${colors.bullet("•")} Non-preview runs can spawn detached sessions, especially long Pro API runs. If the CLI times out, do not re-run — reattach with ${colors.accent("oracle session <slug>")} to resume/inspect the existing run.`,
